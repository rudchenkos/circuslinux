...
int game(void)
{
  ...
  do
      ...
      while (SDL_PollEvent(&event) > 0)
	{
	  ...
	  if (keystate[SDL_SCANCODE_LEFT] || axis < -256)
	    {
	      teeter_xmm = -2;
	      if (teeter_xm > 0)
		teeter_xm = 0;
	    }
	  else if (keystate[SDL_SCANCODE_RIGHT] || axis > 256)
	    {
	      teeter_xmm = 2;
	      if (teeter_xm < 0)
		teeter_xm = 0;
...
void setup(void)
{
  ...
  window = SDL_CreateWindow("Circus Linux!",
  ...
  for (i = 0; i < NUM_IMAGES; i++)
    {
      ...
      if (SDL_SetColorKey(image, SDL_TRUE,
